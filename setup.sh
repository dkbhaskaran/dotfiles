...
function run_tmux() {
  if [ -z "$TMUX" ]; then
    ...
    if [ $det_sess_cnt -gt 0 ]; then
      ...
      count=0
      while true; do
        tmux has-session -t "session-$count "
        if [ $? -ne 0 ]; then
          break
        fi
        count=$((count+1))
      ...
      tmux -f ~/.tmux.conf -u -2 new -s "session-$count "
    fi
  fi
}
...
function drun() {
  ...
  docker run -d -it --name=docker-${count} --network=host --device=/dev/kfd --device=/dev/dri --group-add video --cap-add=SYS_PTRACE --security-opt seccomp=unconfined --ipc=host -v $HOME/dockerx:/dockerx $1
}
